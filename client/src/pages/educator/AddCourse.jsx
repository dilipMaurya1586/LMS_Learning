...
//       const token = await getToken();
//       const { data } = await axios.post(
//         backendUrl + "/api/educator/add-course", formData, { headers: { Authorization: `Bearer ${token}` } }
//       );
...
//               <div className='flex justify-between items-center p-4 border-b'>
//                 <div className='flex items-center'>
//                   <img onClick={() => handleChapter('toggle', chapter.chapterId)} width={14}
//                     className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`} src={assets.dropdown_icon} alt="" />
//                   <span className='font-semibold'> {chapterIndex + 1} {chapter.chapterTitle} </span>
//                 </div>
//                 <span className='text-gray-500'>{chapter.chapterContent.length} Lectures</span>
//                 <img onClick={() => handleChapter('remove', chapter.chapterId)} className='cursor-pointer' src={assets.cross_icon} alt="" />
...
//                   <button className='w-full bg-blue-400 text-white px-4py2 rounded' onClick={addLecture} type='button'>Add</button>
//                   <img onClick={() => setShowPopup(false)} className='absolute top-4 right-4 w-4 cursor-pointer' src={assets.cross_icon} alt="cross_icon" />
//                 </div>
//               </div>
//             )
//           }
